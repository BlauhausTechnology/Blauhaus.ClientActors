...
{
    public class GetModelAsyncTests : BaseActorTest<ModelActorContainer<ITestModelActor, Guid, ITestModel>>
    {
        ...
        {
            //Act
            var result = await Sut.GetModelAsync(_id);

            //Assert
            MockTestActor.Mock.Verify(x => x.InitializeAsync(_id));
            Assert.That(result, Is.EqualTo(_mockTestModel.Object));
        }
